	...
	while (format && format[i])
	{
		if (format[i] == '%')
		{
			...
			if (format[i] == 'd' || format[i] == 'i')
				...
				count += print_base(va_arg(args, unsigned int), 10, 0, 0, zero, minus, width);
			else if (format[i] == 'o')
				count += print_base(va_arg(args, unsigned int), 8, hash, 0, zero, minus, width);
			else if (format[i] == 'x')
				count += print_base(va_arg(args, unsigned int), 16, hash, 0, zero, minus, width);
			else if (format[i] == 'X')
				count += print_base(va_arg(args, unsigned int), 16, hash, 1, zero, minus, width);
			else if (format[i] == 'c')
